...
Page({
  ...
  onLoad: function (options) {
    ...
    this.setData({ college: collegeList })
    //表单验证规则
    const rules = {
      name: {
        required: true,
  ...
  bindcollege: function (e) {
    ...
    if (newSource.length != 0) {
      this.setData({ bindCollege: newSource });

    } else {
      this.setData({ bindCollege: [], major: [] });
    }
  },
  /**
  ...
  collegeitemtap: function (e) {
    this.setData({ inputCollege: e.target.id, bindCollege: [] });
  ...
  bindmajor: function (e) {
    ...
    if (newSource.length != 0) {
      this.setData({ bindMajor: newSource })
    } else {
      this.setData({ bindMajor: [] })
    }
  },
  /**
  ...
  majoritemtap: function (e) {
    this.setData({ inputMajor: e.target.id, bindMajor: [] })
  },
  /**
   * 提交表单
   */
  formSubmit: function (e) {
    ...
    if (!this.WxValidate.checkForm(e)) {
      ...
      //提示信息
      this.setData({ errorMsg: error[0].msg, showErrMsg: 'showError' });
      return false
    }
    this.setData({ errorMsg: '', showErrMsg: '' });
    ...
    wx.showModal({
      ...
      success: function (res) {
        ...
        if (res.confirm) {
          that.data.sure = true;
          that.setData({ userNumber: e.detail.value.number });
          wx.request({
            url: 'https://www.jluibm.cn/jluibm-wx/check-enroll.php?request=checkAlready&{}',
            method: "POST",
            data: {
              number: e.detail.value.number,
              enroll_id:app.globalData.enroll_ID,     
            },
            header: {
              ...
              if (res.data.message == "has_enrolled") {
                ...
                wx.request({
                  ...
                  data: {
                    ...
                    comeFrom: e.detail.value.comeFrom
                  },
                  ...
                  success: function (res) {
                    console.log(res)
                    if (res.data.message){
                      wx.showModal({ title: '提示', content: '报名成功！', showCancel: 'false' });
                    }
                    else if (res.data.message =="error"){
                      wx.showModal({ title: '提示', content: '报名失败了哦，请检查报名信息是否正确！', showCancel: 'false' });
                    }
            ...
            fail: function (err) {
              ...
              wx.showToast({ title: '出错啦\n再试一次吧', icon: 'loading', duration: 2000, mask: true })
            }
          });
        }
        if (res.cancel) {
